...
module.exports = function getChatCommand(args, command, message) {
  ...
  switch (command) {
    ...
    case "remind":
      if (!args.length) {
        return message.channel.send(
          "You need to supply seconds and reminder message!"
        );
      }
      var query = args.join(" ");
      qStr = query.split(" ");
    ...
    case "account":
      if (!args.length) {
        return message.channel.send(
          "You need to supply a BattleTag! (ex: WhiskeyRomeo#1730)"
        );
      }
    ...
    case "hero":
      if (!args.length) {
        return message.channel.send(
          "You need to supply a BattleTag and HeroName! (ex: !hero WhiskeyRomeo#1730 BackClapper)"
        );
      }
    ...
    case "hcl":
      if (!args.length) {
        return message.channel.send(
          "You need to supply a BattleTag, HeroName, and Season! (ex: !hcl WhiskeyRomeo#1730 BackClapper 20)"
        );
      }
    ...
    case "skill_detail":
      if (!args.length) {
        return message.channel.send(
          "You need to supply a Class Type and Skill Name (ex: !skill_detail barbarian bash)"
        );
      }
    ...
    case "skill_list":
      if (!args.length) {
        return message.channel.send(
          "You need to supply a Class Type (ex: !skill_list barbarian)"
        );
      }
