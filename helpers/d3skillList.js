...
module.exports = async function dskillList(message, args, blizz_auth) {
  ...
  const data = await fetch(
    `https://us.api.blizzard.com/d3/data/hero/${queryOne.toLowerCase()}?locale=en_US&access_token=` +
      blizz_auth
  ).then((response) => {
  ...
  const embed = new MessageEmbed()
    .setColor("#FF0000")
    .setDescription(
      "Use !skill_detail with class and skill name for additional details\n" +
